...
while añadido != 'T':
    if añadido == 'A':
        factura= input('Introduce el número de la factura: ')
        coste = float(input('Introduce el coste de la factura: '))
        facturas[factura] = coste
        remains += coste
    if añadido == 'P':
        factura = input('Introduce el número de la factura a pagar: ')
        añadido  = facturas.pop(factura, 0)
        recolectado += añadido
        remains -= añadido
    print('Recaudado:', recolectado)
    print('Pendiente de cobro: ', remains)
    añadido = input('¿Quieres añadir una nueva factura (A), pagarla (P) o terminar (T)? ')